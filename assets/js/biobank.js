// biobank.js — renders the Biobank tracker from assets/data/biobank.json
// (built daily by the update-biobank GitHub Action). Filter by category,
// search, sort by date. Reuses the .deal-table / .filter-pill styling.

(function () {
  const tableMount = document.getElementById("bbTable");
  if (!tableMount) return;
  const statusEl  = document.getElementById("bbStatus");
  const searchEl  = document.getElementById("bbSearch");
  const updatedEl = document.getElementById("bbUpdated");
  const countEl   = document.getElementById("bbCount");
  const catBox    = document.getElementById("bbCategory");
  const DATA_URL  = "/assets/data/biobank.json";
  ...
  setStatus("Loading biobank updates…", false);
  fetch(DATA_URL, { cache: "no-cache" })
    ...
    .catch(() => setStatus("Couldn't load the biobank data yet. It is generated by the daily update job — check back after the first run.", true));
})();
