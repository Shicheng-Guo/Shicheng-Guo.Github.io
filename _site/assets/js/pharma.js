// pharma.js — renders the Pharma 4.0 / 5.0 / 6.0 tracker from
// assets/data/pharma.json (built daily by the update-pharma GitHub Action).
// Filter by category, search, sort by date. Reuses .deal-table / .filter-pill.

(function () {
  const tableMount = document.getElementById("phTable");
  if (!tableMount) return;
  const statusEl  = document.getElementById("phStatus");
  const searchEl  = document.getElementById("phSearch");
  const updatedEl = document.getElementById("phUpdated");
  const countEl   = document.getElementById("phCount");
  const catBox    = document.getElementById("phCategory");
  const DATA_URL  = "/assets/data/pharma.json";
  ...
  function renderTable() {
    ...
    const head = el("tr", null,
      el("th", { class: "deal-th-date", onclick: () => { sortDesc = !sortDesc; renderTable(); }, title: "Sort by date" },
        "Date " + (sortDesc ? "▾" : "▴")),
      el("th", null, "News · Perspective · Comment"),
      el("th", null, "Category"),
    );
  ...
  setStatus("Loading Pharma 4.0/5.0 updates…", false);
  fetch(DATA_URL, { cache: "no-cache" })
    ...
    .catch(() => setStatus("Couldn't load the Pharma 4.0/5.0 data yet. It is generated by the daily update job — check back after the first run.", true));
})();
